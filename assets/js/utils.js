/* =========================================================
   Shared utilities: formatting, rendering, icons
   ========================================================= */
...
const Utils = {
  formatPrice(n) {
    ...
    return num.toLocaleString('ar-EG', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) + ' ' + (s.currency || 'د.أ');
  },
  formatNumber(n) { return Number(n).toLocaleString('ar-EG'); },
  formatDate(d) {
  ...
  stars(rating) {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      else html += '<span style="color:#e0dcd0">★</span>';
    }
    return html;
  },
  statusLabel(s) {
    return ({ ordered: 'تم الطلب', received: 'قيد التجهيز', preparing: 'قيد التجهيز', out_for_delivery: 'قيد التوصيل', delivered: 'تم التسليم', cancelled: 'ملغي' })[s] || s;
  },
  statusColor(s) {
    return ({ ordered: 'tag-blue', received: 'tag-gold', preparing: 'tag-gold', out_for_delivery: 'tag-purple', delivered: 'tag-green', cancelled: 'tag-red' })[s] || 'tag-gray';
  },
  statusSteps() {
    return [
  ...
  toast(message, type = 'success') {
    ...
    t.innerHTML = '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><path d="M9 12l2 2 4-4"/></svg>';
  ...
  icon(name, size = 20) {
    const icons = {
      cart: '<svg width="' + size + '" height="' + size + '" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="9" cy="21" r="1"/><circle cx="20" cy="21" r="1"/><path d="M1 1h4l2.68 13.39a2 2 0 0 0 2 1.61h9.72a2 2 0 0 0 2-1.61L23 6H6"/></svg>',
      heart: '<svg width="' + size + '" height="' + size + '" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg>',
      heart_filled: '<svg width="' + size + '" height="' + size + '" viewBox="0 0 24 24" fill="currentColor" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg>',
      ...
      trash: '<svg width="' + size + '" height="' + size + '" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-2 14a2 2 0 0 1-2 2H9a2 2 0 0 1-2-2L5 6"/><path d="M10 11v6M14 11v6"/></svg>',
      ...
      refresh: '<svg width="' + size + '" height="' + size + '" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="1 4 1 10 7 10"/><polyline points="23 20 23 14 17 14"/><path d="M20.49 9A9 9 0 0 0 5.64 5.64L1 10m22 4l-4.64 4.36A9 9 0 0 1 3.51 15"/></svg>',
      ...
      snapchat: '<svg width="' + size + '" height="' + size + '" viewBox="0 0 24 24" fill="currentColor"><path d="M12.166 2.205c.05-.001.106-.003.159-.003l.16.003c.61.014 1.225.145 1.79.385.564.24 1.04.594 1.388 1.027.348.434.567.948.638 1.495.07.547.022 1.107-.144 1.632-.166.526-.45 1.005-.828 1.4-.378.394-.84.692-1.349.871l.026.176c.077.504.21.997.395 1.469.184.473.421.92.706 1.331.286.41.62.78.998 1.103.378.322.797.595 1.247.812.45.217.928.376 1.42.474.493.098 1 .135 1.504.111.504-.024 1.003-.108 1.485-.247l.05.18c-.39.276-.832.471-1.298.575-.467.105-.952.128-1.426.07l-.1.156c.05.273.13.539.24.795.108.255.243.498.402.723.158.225.34.43.541.611.202.182.422.341.657.474l-.06.18c-.295-.026-.582-.092-.855-.198-.272-.105-.528-.243-.762-.41-.234-.166-.446-.36-.631-.578-.185-.217-.343-.456-.47-.71l-.144.052c.04.215.054.434.04.652-.014.218-.054.434-.118.642-.064.208-.152.408-.262.595-.11.187-.242.36-.392.515l-.16-.072c.06-.205.094-.418.1-.632.007-.214-.013-.428-.06-.636l-.16-.026c-.13.305-.305.589-.518.842-.213.253-.464.473-.742.65-.278.178-.582.316-.9.41-.32.094-.652.141-.987.141-.335 0-.667-.047-.987-.141-.318-.094-.622-.232-.9-.41-.278-.177-.529-.397-.742-.65-.213-.253-.388-.537-.518-.842l-.16.026c-.047.208-.067.422-.06.636.006.214.04.427.1.632l-.16.072c-.15-.155-.282-.328-.392-.515-.11-.187-.198-.387-.262-.595-.064-.208-.104-.424-.118-.642-.014-.218 0-.437.04-.652l-.144-.052c-.127.254-.285.493-.47.71-.185.218-.397.412-.631.578-.234.167-.49.305-.762.41-.273.106-.56.172-.855.198l-.06-.18c.235-.133.455-.292.657-.474.201-.181.383-.386.541-.611.159-.225.294-.468.402-.723.11-.256.19-.522.24-.795l-.1-.156c-.474.058-.959.035-1.426-.07-.466-.104-.908-.299-1.298-.575l.05-.18c.482.139.981.223 1.485.247.504.024 1.011-.013 1.504-.111.492-.098.97-.257 1.42-.474.45-.217.869-.49 1.247-.812.378-.323.712-.693.998-1.103.285-.411.522-.858.706-1.331.185-.472.318-.965.395-1.469l.026-.176c-.51-.179-.97-.477-1.35-.871-.377-.395-.66-.874-.827-1.4-.166-.525-.215-1.085-.144-1.632.07-.547.29-1.061.638-1.495.347-.433.823-.787 1.387-1.027.565-.24 1.18-.371 1.79-.385z"/></svg>',
      ...
      arrow_right: '<svg width="' + size + '" height="' + size + '" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="19" y1="12" x2="5" y2="12"/><polyline points="12 19 5 12 12 5"/></svg>',
      ...
      chart: '<svg width="' + size + '" height="' + size + '" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="12" y1="20" x2="12" y2="10"/><line x1="18" y1="20" x2="18" y2="4"/><line x1="6" y1="20" x2="6" y2="16"/></svg>',
      ...
      chevron_left: '<svg width="' + size + '" height="' + size + '" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="15 18 9 12 15 6"/></svg>',
      ...
      invoice: '<svg width="' + size + '" height="' + size + '" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/></svg>',
